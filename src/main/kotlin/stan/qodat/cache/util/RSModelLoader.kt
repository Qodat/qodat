...
import stan.qodat.cache.definition.ModelTextureDefinition
import stan.qodat.cache.impl.oldschool.RS2ModelDefinition
...
class RSModelLoader {
        ...
        when {
            isRS3(data) -> {
                try {
                    ...
                    logger.severe("Failed to load model $modelId as a RS3 model")
                    logger.info("Trying to load as RS2 High rev model")
                    try {
                        loadHighRev(modelId, data)
                    } catch (e2: Exception) {
    ...
    private fun loadHighRev(modelId: Int, data: ByteArray): ModelDefinition {
        ...
        val definition = RS2ModelDefinition()
    ...
    companion object {
        ...
        fun isHighRev(data: ByteArray) =
            data[data.size - 1] == (-1).toByte()
                    && data[data.size - 2] == (-1).toByte()
        fun isRS3(data: ByteArray) = data[0] == 1.toByte() && data[1] == 1.toByte()
    }
